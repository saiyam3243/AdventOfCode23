...
iteration = 0
skip = True
split_line = """Game 1: 7 blue, 4 red, 11 green; 2 red, 2 blue, 7 green; 2 red, 13 blue, 8 green; 18 blue, 7 green, 5 red
Game 2: 3 green, 4 red, 4 blue; 6 red, 4 green, 4 blue; 2 lue, 4 green, 3 red
...
for game_no in games:
    ...
    for i in game: #i is the game, 100 games
        if skip:
            skip = False
            continue
        bags = i.split(";")
        for j in bags: #j is the bag, any number of bags

            colors = j.split(",")

            for k in colors: #k is the color and number present in the bag, max 3 iterations
                # final = ['', '7', 'blue']
                final = k.split(" ")
                number = int(final[1])
                color = final[2]
