...
def sum_of_lines(index, list, answer, count):
    start = index
    end = index
    if list[index-1].isdigit():
        # if list[index].isdigit():
        #     pass
        # else:
        #     end = index-1
        if list[index-2].isdigit():
            if list[index-3].isdigit():
                start = index-3
            else:
                start = index-2
        else:
    ...
    int_list = re.findall(r"\b\d\b|\b\d\d\b|\b\d\d\d\b", list[start:end+1])
    #prod = 1
    for i in int_list:
        answer *= int(i)
    #answer += prod
    replacement_string = '<' * (end-start+1)
    # list = list.replace(list[start:end+1], replacement_string,1)

    # for j in int_list:
    #     list = list[:start] + replacement_string + list[end+1:]
    print(len(list))
    count += len(int_list)
    return list , answer, count
...
# ....33...99..
# """
lines = split_line.split("\n")
answer = 0
for i in range(1,len(lines)):
    while re.search("[*]", lines[i]) != None:
        
        find_splchar = re.search("[*]", lines[i])
        if find_splchar:
            index = find_splchar.start()
            s = (lines[i])[index]
            print(s)
            ...
            lines[i-1], prod, count = sum_of_lines(index, lines[i-1], prod, count)
            if count > 2:
                continue
            ...
            if count > 2:
                continue
            lines[i], prod, count = sum_of_lines(index, lines[i], prod, count)
            if count > 2:
                continue
            if count == 2:
                answer += prod
